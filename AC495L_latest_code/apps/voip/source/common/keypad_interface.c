    ...
    case UI_IDLE_STATE:
        ...
        case IDLE_PROCESS:
            if (cInkey >= P2P_DIGIT0_LOWER_VALUE && cInkey <= P2P_DIGIT0_UPPER_VALUE)
            {
                ...
                memcpy(g_arcLCDMsg, "P2P to:            ", TWENTY);

                sprintf(&g_arcLCDMsg[SEVEN], "%d", g_iarcDialed_Key_Array[g_ucDialed_Sequence_No]);

                print_msg_on_lcd(IOCTL_PRINT, g_arcLCDMsg, LINE2, ZERO, LCD_CLR_SECOND_LINE);
            ...
            else
            {
                ...
                case CONF_KEY:
                    printf("CONF KEY PRESSED \n");
                    break;

                case AIM_KEY:
                    printf("AIM_KEY KEY PRESSED \n");
                    break;
                case ENTER_KEY:
                    printf("ENTER_KEY KEY PRESSED \n");
                    break;
                case EPBX_KEY:
                    printf("EPBX_KEY KEY PRESSED \n");
                    break;
                case PRIO_KEY:
                    printf("PRIO_KEY KEY PRESSED \n");
                    break;

                case VOL_UP_KEY:
                    printf("VOL_UP_KEY KEY PRESSED \n");
                    volume_up_handle(PG_CHANNEL);
                    break;

                case VOL_DN_KEY:
                    printf("VOL_DN_KEY KEY PRESSED \n");
        ...
        case P2P_KEY_DIALLING_MODE:
            ...
            case RESET_KEY:
                ...
                print_msg_on_lcd(IOCTL_PRINT, "RESET_KEY", LINE2, ZERO, LCD_CLR_SECOND_LINE);
            ...
            default:
                ...
                case DIGIT1:
                    if (cInkey >= DIGIT_0 && cInkey <= DIGIT_9)
                    {
                        ...
                        print_msg_on_lcd(IOCTL_PRINT, g_arcLCDMsg, LINE2, ZERO, NO_LCD_CLR);
            ...
            default:
                ...
                case DIGIT0:
                    if (cInkey >= DIGIT_1 && cInkey <= DIGIT_4)
                    {
                        ...
                        // update_lcd_msg(g_IcomLcdMsg,LINE2,LCD_CLR_SECOND_LINE,0);
                        // send_msg_ui(IDLE_STATE,LCD,LINE2,CONTINUE_DISPLAY,g_IcomLcdMsg,LCD_CLR_SECOND_LINE,0);
                        print_msg_on_lcd(IOCTL_PRINT, g_IcomLcdMsg, LINE2, ZERO, LCD_CLR_SECOND_LINE);
                        g_ucDialed_Sequence_No++;
                        break;
                    }
                    else
                    {
                        ...
                        print_msg_on_lcd(IOCTL_PRINT, "INVALID_KEY", LINE2, ZERO, LCD_CLR_SECOND_LINE);
                        clear_validation_processing();
...
void keypad_report(keypad_key, pressed)
{
    ...
    if (pressed == EVENT_KEY_UP)
    {
        ...
        if (keypad_key != 52)
        {
            ...
            {
                printf("in g_valid_key_sequence %d\n", g_iarcDialed_Key_Array[0]);
