...
//  int i_ret,sre_ret,i_ptt_pressed=0,i_cnt=0;
...
/*int update_lcd_msg_1(char *data,int line_no,int clear_lcd,int position)
{
    /*int fd;
            ...
            /*  if(g_iInterdigit_timer)
             ...
             /* ///SRE MUTE activation 12-03-22
               ...
               // sre_ret=operate_gpio(SRE_MUTE_ACT,READ,CLEAR);
                sre_ret=read_from_io(SRE_MUTE_ACT);///
               // printf("*******sre_ret=%d******\n",sre_ret);
                if(sre_ret==SET)
                {
...
/*              pthread_mutex_lock(&lock_call_type);
...

                {
                    ...
                    i_ret= read_from_io(HOOK_SW_SENSE);
      ...
      /*  if(i_fd == g_iconnection_fd) //if(g_uiUi_socket_fd)
        {
         // i_rc= read(i_fd,&c_Fsu_state,sizeof(char));
          printf("UI FSU STATE IS = %c %s %d \n",c_Fsu_state,c_Fsu_state,c_Fsu_state);
          int rd;
