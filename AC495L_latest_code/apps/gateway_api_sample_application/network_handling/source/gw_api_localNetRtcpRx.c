 ...
 *                                                                  *
 * File Name:	gw_api_localNetRtcpRx.c                             *
 *                                                                  *
...
void *acl_RTCPrxJob(void *arg)
{
	...
	/*see very important comment in gw_api_localNetRx.c*/
	tv.tv_sec = 5;
   	tv.tv_usec = 0;
	...
	for(;;) 
	{
 	        ...
 	        for(curFd = 0; curFd < sFdDBRealSize; curFd++) 
		{
			...
			{ 
				/* we got one!! */
				if((buf_len=recv(sFdDB[curFd], rtcp_rx_message, ACL_MSG_SIZE, 0)) < 0) 
				{
					printf("\nacl_RTCPrxJob:: recvfrom");
				}
