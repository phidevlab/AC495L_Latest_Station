...
typedef struct _RvSdpAttribute {
	...
	const RvSdpSpecAttributeData	*iSpecAttrData; /* if the pointer is set the
													 ...
													 * ConnMode etc) */
    RvUint32                iLabel;                 /* attribute label */
} RvSdpAttribute;
...
#define RV_SDP_MAX_NUM_OF_MID_ID_IN_LIST 10

/*  GROUP ATTR :
 *	media group special attributes 'a=group:'
 *  The syntax is: 'a=group:semantics *(space Id_tag)'
 *  This attribute can apear only in msg level. 
 *  It can apear a few times in a message but with diffeent semantics.
 *
 *  media Id special attributes : 'a=mid 1*(space Id_tag)' is RvSdpAttribute. 
 *  media Id label (a=mid) can appear only in the
 *  media desc level,and only single appearence per media.the Id_tag should 
 *  be composed of not more then 4 chars.and should be unique number in the SDP session desc.                                            
 */
typedef struct _RvSdpMediaGroupAttr
{
    RvSdpAttribute          iGroupAttribute;	
#if defined(RV_SDP_CHECK_BAD_SYNTAX)
    RvSdpString             iBadSyntaxField;
#endif
    RvSdpString             ieSemanticsType;	/*  str semantics can be: LS /SRF /FID */
    RvSdpString             iMidIdParams[RV_SDP_MAX_NUM_OF_MID_ID_IN_LIST];/* media Id's list defined as a group */
    RvUint16                iMidIdParamsNum;   /* num of params in the mid tag list*/
    void*                   iObj;			   /* allocator or message instance */
    RvAlloc*                iAlloc;			   /* the allocator used for RvSdpMediaGroupAttr*/ 

} RvSdpMediaGroupAttr;

#endif /*#ifdef RV_SDP_MEDIA_GROUPING_ATTR */
...
#define RV_SDP_PRECOND_MAX_PRECOND_NAME 5

/*  PRECONDITION ATTR :
 *	precondition special attributes covers 3 type of attributes:
 *  'a=curr:' ,'a=des' & 'a=conf'
 ...
 */
typedef struct _RvSdpPreconditionAttr
{
    RvSdpAttribute          iPreconditionAttribute;	
#if defined(RV_SDP_CHECK_BAD_SYNTAX)
    RvSdpString             iBadSyntaxField;
#endif
    RvSdpString             iPrecondType;	/*  precond.type can be: QOS */
    RvSdpString             iStatusType;   /*  Status Type can be:'e2e'/'remote'/'local' */
    RvSdpString             iDirectionTag; /* Direction tag can be:'none'/'send'/'recv'/'sendrecv'*/
    RvSdpString             iStrengthTag;  /* Strength tag can be:'none'/'mandatory'/'failure'/'unknown'*/
    RvSdpPrecondName        iePrecondName;  /* enum of the precond name*/
   
    void*                   iObj;			/* allocator or message instance */
    RvAlloc*                iAlloc;			/* the allocator used for RvSdpMediaGroupAttr*/ 

} RvSdpPreconditionAttr;

#endif /*#ifdef RV_SDP_PRECONDITIONS_ATTR */
...
/*
 *	rtcp special attributes 'a=rtcp:'
 */
typedef struct _RvSdpRtcp
{
    RvSdpAttribute          iRtcpAttribute;	
    RvInt32                 iPort;          /* RTCP session port number */
    RvSdpString             iNetTypeStr;    /* network type */
    RvSdpString             iAddrTypeStr;   /* address type */
    RvSdpString             iAddress;       /* RTCP session address: multicast,or FQDN  */
    RvAlloc*                iAlloc;		    /* the allocator used for RvSdpRtcp 
											 *  instance allocation */
    void*                   iObj;		    /* allocator or message instance */
} RvSdpRtcp;
...
#define RV_SDP_MEDIA_VCID_MAX       4
...
typedef struct _RvSdpMediaDescr {
    ...
    RvInt32                 iPort;               /* media port number 
                                                    or special MEGACO values 
                                                    RV_SDP_PORT_IGNORE/RV_SDP_PORT_ANY/RV_SDP_PORT_ATM*/
    RvSize_t                iNumOfPorts;        /* number of subsequent ports */
    ...
    RvSdpVcId               iMediaVcIdTypes[RV_SDP_MEDIA_VCID_MAX];
    RvSdpString             iMediaVcIdValues[RV_SDP_MEDIA_VCID_MAX];
    RvUint16                iMediaVcIdsNum;

    RvSdpString             iMediaNASCtrlMethod;
