...
typedef enum
{
	...
	RVSIP_CONTENT_TYPE_TYPE_PARAM_MEDIATYPE,
	RVSIP_CONTENT_TYPE_TYPE_PARAM_MEDIASUBTYPE
} RvSipContentTypeHeaderStringName;

#ifdef RV_SIP_JSR32_SUPPORT
/*
 * RvSipContentTypeHeaderFieldName
 * ----------------------------
 * This enum defines all the Content-Type header fields.
 * It is used for getting and setting via RvSipHeader interface.
 */
typedef enum
{
	RVSIP_CONTENT_TYPE_FIELD_MEDIATYPE      = 0,
	RVSIP_CONTENT_TYPE_FIELD_MEDIASUBTYPE   = 1,
	RVSIP_CONTENT_TYPE_FIELD_BOUNDARY       = 2,
	RVSIP_CONTENT_TYPE_FIELD_VERSION        = 3,
	RVSIP_CONTENT_TYPE_FIELD_BASE           = 4,
	RVSIP_CONTENT_TYPE_FIELD_OTHER_PARAMS   = 5,
	RVSIP_CONTENT_TYPE_FIELD_BAD_SYNTAX     = 6
}RvSipContentTypeHeaderFieldName;
 ...
 * ------------------------------------------------------------------------
 * General: Retrieve the Start parameter of type address from the Content-Type header
 * Return Value: Returns RvStatus.
 * ------------------------------------------------------------------------
 * Arguments:
 * input: hHeader       - Handle to the Content-Type header.
 * output: phStartAddr  - Pointer to an address handle which is the type of Start parameter
 ***************************************************************************/
 ...
 *          The given address is copied into the Content-Type header.
 * Return Value: Returns RvStatus.
 * ------------------------------------------------------------------------
 * Arguments:
 *    input: hHeader    - Handle to the Content-Type header.
			 hStartAddr - Handle to the Start parameter (of address type). 
 ***************************************************************************/
 ...
 * ------------------------------------------------------------------------
 * General: Retrieve the media type enumeration value of the 'type' parameter. 
 *			If RVSIP_MEDIATYPE_OTHER
 *          is returned, you can receive the media type string using
 *          RvSipContentTypeHeaderTypeParamGetStrMediaType().
 * Return Value: Returns the media type of the Content-Type 'type' parameter.
 * ------------------------------------------------------------------------
 * Arguments:
 * input: hHeader    - Handle to the Content-Type header.
 ***************************************************************************/
 ...
 *          a given buffer. Use this function when the media type enumeration
 *          returned by RvSipContentTypeHeaderTypeParamGetMediaType() is RVSIP_MEDIATYPE_OTHER.
 *          If the bufferLen is adequate, the function copies the requested
 *          parameter into strBuffer. Otherwise, the function returns
 *          RV_ERROR_INSUFFICIENT_BUFFER and actualLen contains the required buffer
 *          length.
 ...
 *          RVSIP_MEDIASUBTYPE_OTHER is returned, you can receive the media
 *          sub type string using RvSipContentTypeHeaderTypeParamGetStrMediaSubType().
 * Return Value: Returns the sub media type of the Content-Type 'type' parameter.
 * ------------------------------------------------------------------------
 * Arguments:
 * input: hHeader    - Handle to the header object.
 ***************************************************************************/
 ...
 *          into a given buffer. Use this function when the media sub type
 *          enumeration returned by RvSipContentTypeHeaderTypeParamGetMediaSubType() is
 *          RVSIP_MEDIASUBTYPE_OTHER.
 *          If the bufferLen is adequate, the function copies the requested
 *          parameter into strBuffer. Otherwise, the function returns
 *          RV_ERROR_INSUFFICIENT_BUFFER and actualLen contains the required buffer
