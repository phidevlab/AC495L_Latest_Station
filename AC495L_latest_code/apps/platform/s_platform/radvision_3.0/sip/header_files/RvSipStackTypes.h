...
typedef struct {
    ...
    RvInt32      coreRcacheLogFilters;
    RvInt32      coreEhdLogFilters;
    RvInt32      coreImsipsecLogFilters;
 ...
 * manualSessionTimer
 * ------------------
 * This parameter is DEPRECATED in version 4.0!!!
 ...
 * The forked call-leg uses the invite-linger-timeout value as an original call-leg.
 *
 ...
 * The application can change the list of DNS domains on runtime. It must specify
 ...
 * bOldInviteHandling
 * -----------------------------
 * Until version 4.0 the Implementation of Invite transaction that received 2xx response
 * was not according to RFC3261, but according to bis 2543.
 * According to RFC3261, an INVITE and its 2xx are a full transaction and the ACK
 * on 2xx should be a new transaction. In older SIP stack versions, ACK was
 * considered as part of the Invite transaction.
 * From this version on, ACK is no longer part of the Invite transaction.
 * The Invite transaction will be terminated when 2xx is received or sent.
 * In order to keep the old non-standard stack behavior you should set the
 * bOldInviteHandling parameter to RV_TRUE.
 *
 * Default Value: RV_FALSE
 * Remarks: Group A parameter
 *
 * bResolveTelUrls
 * ---------------------
 * Indicates that tel: schemes should be resolved by the stack. 
 * version 4.0 implements the ENUM/TEL URL scheme. 
 * If this parameter is set to RV_TRUE, the stack will try to resolve tel 
 * URLs according to RFC3764.
 * If this parameter is set to RV_FALSE, the stack will call the 
 * RvSipXXXOtherURLAddressFoundEv callback if the remote address contains a tel URLs
 * scheme.
 * Note: If you wish the stack to resolve tel URLs you should also compile the stack with
 * the RV_SIP_TEL_URI_SUPPORT compilation flag.
 * Default Value: RV_FALSE
 * Remarks: Group A parameter
 *
 * strDialPlanSuffix
 * -----------
 * A string supplied by the user to indicate the dialing plan DNS suffix.
 * This is the suffix that will be added for the uri discovery NAPTR query.
 * e.g. "e164.arpa"
 * Default Value: NULL
 * Remarks: Group A parameter
 *
 ...
 * -------------------------
 * Determines the recommended percentage of opened connections that the stack is allowed to hold 
 * at any given moment from its pool of connections.
 * RFC 3261 recommends that connections will be kept open for some period of time after the last 
 * message was exchanged over the connection; however, the exact time period to leave the connection 
 * open is implementation defined.
 * Using the connectionCapacityPercent configuration parameter the Sip Stack enables the application 
 * to leave client connections open even after the connections are no longer in use.
 * When the connectionCapacityPercent parameter is greater than 0, the SIP Stack will not close 
 * connections that are no longer in use. Such connections will be kept in a separate list and will 
 * remain opened as long as their resources are not required.
 * Once the percentage of opened connections exceeds the allowed connectionCapacityPercent the 
 * Sip Stack will start closing connection from this list each time it is required to open a new connection.
 *
 * Default value: 0.
 * Remarks: Group A parameter
 *
 * numOfSctpAddresses
 * -------------------------
 * The number of SCTP addresses to which the application whishes to listen.
 * Setting this number to 0 means that the application does not want to listen
 * to any SCTP addresses.
 * It is the application's responsibility to allocate two arrays with
 * (numOfSctpAddresses) cells that contains addresses and corresponding ports.
 * Default Value: 0
 * Remarks: Group A parameter
 *
 * localSctpAddresses
 * localSctpPorts
 * -----------------------------
 * Local SCTP addresses on which the SIP stack will listen.
 * These parameters will be use in multi-homed host applications.
 *
 * Default value: NULL (no SCTP addresses)
 * Remarks: Group A parameter
 *
 * Note: localSctpAddresses array should be allocated by the application
 *       according to the size given in numOfSctpAddresses.
 *       localSctpPorts array should be allocated by the application according
 *       to the size given in numOfSctpAddresses.
 *       Each entry in the localSctpAddresses array must be allocated as well,
 *       in order to contain the list of SCTP multihoming IPs.
 *       The Stack can process maximum of
 *       RVSIP_TRANSPORT_SCTP_MULTIHOMED_ADDR_NUM_AVG_PER_CONN multihoming IPs
 *       in each entry. They should be separated by ';'.
 *       Each of the entries of localSctpPorts must be allocated as well,
 *       in order to contain the list of ports, to which the local addresses
 *       will be bound.
 *
 * numOfOutStreams
 * -------------------------
 * Defines number of outbound SCTP streams, set for each local SCTP address.
 * Default value: 1.
 * Remarks: Group A parameter
 *
 * numOfInStreams
 * -------------------------
 * Defines number of inbound SCTP streams, set for each local SCTP address.
 * Default value: 10.
 * Remarks: Group A parameter
 *
 * avgNumOfMultihomingAddresses
 * -------------------------
 * Defines the average number of both local and remote multihoming SCTP IPs per
 * SCTP connection or local address. Each SCTP address is taken from a pool of addresses.
 * The avgNumOfMultihomingAddresses Affects on size of the SCTP multihoming
 * address pool.
 * Default value: RVSIP_TRANSPORT_SCTP_MULTIHOMED_ADDR_NUM_AVG_PER_CONN.
 * Remarks: Group A parameter
 *
 * bIgnoreLocalAddresses
 * -----------------
 * By default the stack is initialized with one UDP and one TCP local address and the application
 * can configure extra local addresses of any transport type.
 * When this parameter is set to RV_TRUE, all local addresses infromation from the configuration is
 * ignored and the stack is constructed with no local addresses and listening sockets.
 * The application can then use the DLA functionality to add local addresses with listening sockets.
 * Default Value: RV_FALSE
 * Remarks: Group A parameter
 *
 * new parameters for version 4.5
 * ===============================

 * maxSecurityObjects
 * -----------------
 * Defines number of Security Object objects.
 * Default value: 1.
 * Remarks: Group A parameter
 *
 * maxIpsecSessions
 * -----------------
 * Defines number of IPsec Session objects.
 * Default value: numOfSecutiryObjects.
 * Remarks: Group B parameter
 *
 * securityLogFilters
 * -----------------
 * Security module log filters.
 * Default Value: 0 - No logging information
 * Remarks: Group A parameter.
 *
 * spiRangeStart - spiRangeEnd
 * ------------------------------
 * Range of SPI values, available for IPsec.
 * Different UEs, using same network interface, should choose different ranges
 * Range [0-255] is reserved for Operation System.
 * Default Value: 0x100 - 0xFFFF
 * Remarks: Group A parameter.
 *
 * secAgreeLogFilters
 * ------------------
 * Security-Agreement module log filters.
 * Default Value: 0 -No logging information
 * Remarks: Group A parameter.
 *
 * maxSecAgrees
 * ------------------
 * The number of security-agreements the SIP stack allocates. 
 * You should set this value to the maximum number of security-agreements 
 * you expect the SIP Stack to handle simultaneously.
 * Default Value: 0
 *
 * new parameters for version 5.0
 * ===============================
 * transcClientLogFilters
 * -------------------
 * transc-Client module log filters.
 * Default Value: 0
 * Remarks: Group A parameter
 *
 * pubClientLogFilters
 * -------------------
 * Publish Client module log filters.
 * Default Value: 0
 * Remarks: Group A parameter
 *
 * maxPubClients
 * -------------
 * The number of Pub-Clients the SIP Stack allocates. You should set
 * this value to the maximum number of publish-clients you expect the
 * SIP Stack to handle simultaneously.
 * Default Value: 2
 * Remarks: Group A parameter
 *
 * regAlertTimeout
 * -------------------
 * Indicates the time in milliseconds that an alert is given, before register-client
 * expiration.
 * Default value: 5000
 * Remarks: Group A parameter
 *
 * pubAlertTimeout
 * -------------------
 * Indicates the time in milliseconds that an alert is given, before publish-client
 * expiration.
 * Default value: 5000
 * Remarks: Group A parameter
 * 
 ...
 * maxAuthObj
 * -------------------
 * Indicates the total number of authentication objects that can be used at the same time for stack objects.
 * 
 * Default value: maxAuthObj = maxRegClient + maxPubClient + maxCallLegs
 * Remarks: Group A parameter
 * 
 ********************************************************************************************/
typedef struct {
...
#ifdef RV_SIP_IMS_ON
	RvInt32				secAgreeLogFilters;
	RvInt32             maxSecAgrees;
    RvInt32             maxSecurityObjects;
    RvInt32             maxIpsecSessions;
    RvInt32             securityLogFilters;
    RvUint32            spiRangeStart;
    RvUint32            spiRangeEnd;
#endif /* #ifdef RV_SIP_IMS_ON */

    /*new parameters for version 5.0*/
	RvInt32  transcClientLogFilters;
	RvInt32  pubClientLogFilters;
#ifndef RV_SIP_PRIMITIVES
	RvInt32  maxPubClients;
	RvInt32  regAlertTimeout;
	RvInt32  pubAlertTimeout;
#endif /*RV_SIP_PRIMITIVES */
