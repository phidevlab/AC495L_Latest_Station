...
typedef enum
{
    ...
    RVSIP_ROUTE_HOP_PATH_HEADER,
    RVSIP_ROUTE_HOP_SERVICE_ROUTE_HEADER
}RvSipRouteHopHeaderType;

#ifdef RV_SIP_JSR32_SUPPORT
/*
 * RvSipRouteHopHeaderFieldName
 * ----------------------------
 * This enum defines all the Route-Hop header fields.
 * It is used for getting and setting via RvSipHeader interface.
 */
typedef enum
{
	RVSIP_ROUTE_HOP_FIELD_ADDR_SPEC    = 0,
    RVSIP_ROUTE_HOP_FIELD_OTHER_PARMAS = 1,
	RVSIP_ROUTE_HOP_FIELD_BAD_SYNTAX   = 2
}RvSipRouteHopHeaderFieldName;
