...
/*-----------------------------------------------------------------------
        S I P  C O N T R O L   C A L L - B A C K  F U N C T I O N S
 ------------------------------------------------------------------------*/

typedef void (*RvSipControlProcessEventF)(RvSipAppCallLegHandle hAppCallLeg,
											RvSipCallLegHandle hCallLeg,
											RvSipCallLegState	state,
											RvSipCallLegStateChangeReason reason);

typedef void (*RvSipControlProcessEventModifyF)(RvSipAppCallLegHandle hAppCallLeg,
										RvSipCallLegHandle hCallLeg,
										RvSipCallLegModifyState       eModifyState,
										RvSipCallLegStateChangeReason eReason);
typedef RvSipAppCallLegHandle (*RvSipControlNewCallLegF)(void);

typedef void (*RvSipControlSetCallParamsF)(RvSipAppCallLegHandle hAppCallLeg,
										   RvSipCallLegHandle hCallLeg, char* to, char* from, char *contact,
										   char* callerName, char*callerDisplayName);

typedef void (*RvSipControlSetMediaF)(RvSipMsgHandle hMsg, RvSipAppCallLegHandle hAppCallLeg);

typedef RvBool (*RvSipControlGetMediaF)(RvSipMsgHandle hMsg, RvSipAppCallLegHandle hAppCallLeg);

typedef void (*RvSipControlMsgReceivedF)(RvSipMsgHandle hMsg, RvSipAppCallLegHandle hAppCallLeg);

typedef void (*RvSipControlReferConnectedF)(RvSipAppCallLegHandle hAppCallLeg, RvSipAppCallLegHandle hNewAppCallLeg);

typedef void (*RvSipControlTransferNotifiedF)(RvSipAppCallLegHandle hAppCallLeg, RvBool isNotifyReceived);
...
typedef struct
{
    ...
    RvBool                          connectMediaOn180;   /* When True, media will be connected both in 180, 183 and 200, 
                                                       when off, media will be connect in 183 and 200 only*/
#ifdef RV_CFLAG_TLS
	RvBool							isTlsEnabled;
#endif
}RvSipControl;
...
#define rvSipControlGetUserDomain(x)			((x)->userDomain)
 ...
 *       RV_OK if succeeded
 *************************************************************************************/
RvStatus rvSipControlForwardCall(RvSipCallLegHandle	hCallLeg, RvChar *sendToAddress);
...
RvBool rvSipControlRegisterClient(RvSipControl*	x,
								  RvCCTerminalSip *		sipTerm);

RvBool rvSipControlUnregisterClient(RvSipControl*			x,
									   RvCCTerminalSip*		sipTerm);
...
RvStatus rvSipControlGetCallLegNonce(IN RvSipCallLegHandle hCallLeg,OUT char* nonce);
