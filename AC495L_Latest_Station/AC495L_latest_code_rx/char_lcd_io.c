...
static void send_data_to_ioexpander(unsigned char *cmd , unsigned char addr)
{
	...
	char cmd_t;
	g_fd_i2c->addr = addr;
	//printk(KERN_INFO "send_data_to_ioexpander->g_fd_i2c->addr=%x temp_buf[0]=%x *cmd=%x\n",g_fd_i2c->addr,temp_buf[0],*cmd);
...
static long i2c_ioctl( struct file *p_file, unsigned int ioctl_command, unsigned long arg,unsigned char *type)
{
	...
	printk(KERN_INFO "ioctl_command: %d\n",ioctl_command);
	...
	switch( (char) ioctl_command ){
		...
		case IOCTL_SET_BIT_IO_EXPANDER:
			send_data_to_ioexpander(ioctl_io_arguments.kbuf_io,ioctl_io_arguments.i2c_device_address);
			break;
