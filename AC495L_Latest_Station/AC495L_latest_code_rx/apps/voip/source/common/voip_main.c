            ...
            if (fd == g_uiApp_timer_fd)
            {
                ...
                else
                {
                    ...
                    if (pvt_call.pvt_call_busy)
                    {
                        ...
                        {
                            printf("g_uiPvt_call_timer is : %d \n", g_uiPvt_call_timer);
                            g_uiPvt_call_timer--;
                            if (g_uiPvt_call_timer == CLEAR)
                            {
                                // printf("g_uiPvt_call_timer is clear\n");
                                printf("PVT_CALL_TIMEOUT set\n");
...
int main(int argc, char **argv)
{
   ...
   // initialize_rows();
    ...
    while (1)
    {
        //unsigned char i2c_row_addr[1] = {0x38};
       ...
       /*ioctl(lcd_io_Fd, IO_READ, &io_msg);
        returned_value = io_msg.kbuf[0];
        printf("**** in returned_value %x \n", returned_value);*/
        // sleep(1);

       /*char init_col_high[] = {0x55};
        io_msg.i2c_device_address = I2C_ROW_ADDRESS;
...

    {
        ...
        if (g_uiFsu_Broadcast_event)
        {
            ...
            case SHUTDOWN_EVENT:
                // printf("SHUTDOWN \n");
                //  g_cInit_app=CLEAR;
                //   send_msg_ui(INIT_STATE,LCD,LINE2,CONTINUE_DISPLAY,"SHUTTING DOWN",LCD_CLR_BOTH_LINE,0);
            ...
            case SERVER_REG_BCAST_EVENT:
                ...
                case VCCS_REGISTERING_STATE:
                    ...
                    // send_msg_ui(VCCS_REGISTERING_STATE,LCD,LINE2,CONTINUE_DISPLAY,"REGISTERING",0,0);
                    send_msg_ui(VCCS_REGISTERING_STATE, LCD, LINE2, CONTINUE_DISPLAY, registering_msg, LCD_CLR_BOTH_LINE, 0);
                    sleep(2);
        ...
        if (g_uiFsu_event)
        {
            ...
            case INIT_STATE:
                ...
                case INIT_EVENT:
                    ...
                    g_uiFsu_event = CLEAR;
                    initialize_fsu();
                    send_msg_ui(INIT_STATE, LCD, LINE1, CONTINUE_DISPLAY, "  VOIP SYSTEM   ", LCD_CLR_BOTH_LINE, 0);
                    send_msg_ui(INIT_STATE, LCD, LINE2, CONTINUE_DISPLAY, "  Phi AudioCom  ", 0, 0);
                    // system("rmmod gpiodrv.ko");
                    // send_msg_ui(INIT_STATE,LCD,LINE1,CONTINUE_DISPLAY,"  VoIP Systems  ",LCD_CLR_BOTH_LINE,0);
                    // send_msg_ui(INIT_STATE,LCD,LINE2,CONTINUE_DISPLAY,"      System    ",0,0);
                    // send_msg_ui(INIT_STATE,LCD,LINE3,CONTINUE_DISPLAY,"    Phi Audiocom   ",0);
                    //   send_msg_ui(INIT_STATE,LCD,LINE4,CONTINUE_DISPLAY,"       INDIA       ",0);
            ...
            case VCCS_REGISTERING_STATE:
                switch (g_uiFsu_event)
                {
                    ...
                    // send_msg_ui(VCCS_REGISTERING_STATE,LCD,LINE2,CONTINUE_DISPLAY,"CHECKINSIDE",LCD_CLR_BOTH_LINE,0);
            ...
            case CONFIGURING_STATE:
                ...
                case CONFIG_RECEIVED:
                    ...
                    //  send_msg_ui(CONFIGURING_STATE,LCD,LINE2,CONTINUE_DISPLAY,"CONFIGURING",0);
                    send_msg_ui(CONFIGURING_STATE, LCD, LINE2, CONTINUE_DISPLAY, configuring_msg, LCD_CLR_BOTH_LINE, 0);
                    sleep(2);
            ...
            case DSP_RADV_INIT_STATE:
                ...
                case ACK_RESPONSE:
                    ...
                    init_dsp();
                    //  g_uiVoipapp_wait_timer=CLEAR;
                    // send_msg_ui(IDLE_STATE,LCD,LINE1,CONTINUE_DISPLAY,stn_config.timetick,CLEAR,0);
                    // send_msg_ui(IDLE_STATE,LCD,LINE1,CONTINUE_DISPLAY,stn_config.timetick,CLEAR,0);
                    // sprintf(g_cLine3_buf,"IDLE %10d",stn_config.logical_id);
                    // send_msg_ui(INIT_STATE,LCD,LINE2,CONTINUE_DISPLAY,"                  ",CLEAR);
                    // send_msg_ui(IDLE_STATE,LCD,LINE3,CONTINUE_DISPLAY,g_cLine3_buf,CLEAR,0);
                    // send_msg_ui(IDLE_STATE,LCD,LINE4,CONTINUE_DISPLAY,stn_config.fcs_desc,CLEAR,0);
                    ...
                    ////operate_gpio(LED_CALL_THROUGH,WRITE,GPIO_OFF);
                    ////operate_gpio(OUT_PROVISION1,WRITE,GPIO_OFF);
                    ////operate_gpio(OUT_PROVISION2,WRITE,GPIO_OFF);
                    ////operate_gpio(OUT_PROVISION3,WRITE,GPIO_OFF);
                    ////operate_gpio(OUT_PROVISION4,WRITE,GPIO_OFF);
                    ////operate_gpio(OUT_PROVISION5,WRITE,GPIO_OFF);
                    ////operate_gpio(OUT_PROVISION6,WRITE,GPIO_OFF);
                    ////operate_gpio(OUT_PROVISION7,WRITE,GPIO_OFF);
                    ////operate_gpio(OUT_PROVISION8,WRITE,GPIO_OFF);
...
*/
                ...
                case APP_TIMEOUT_EVENT:
                    ...
                    if (g_cReceived_key_seq[0] == P2P)
                    {
                        if (p2p_req_retransmission_count == 0)
                        {
                            send_req_msg_to_server(0, last_req_p2p_sent_to_server, P2P_CALL_FRAME_LENGTH, '0', '0');
                            printf("*****Retransmission of p2p call frame to server from APP_TIMEOUT_EVENT******\n");
                            g_uiVoipapp_wait_timer = SERVER_RESPONSE_TIMEOUT;
                            p2p_req_retransmission_count = 1;
                            break;
                        }
                        // send_to_phone_input_cb(ONE,PHONE_INPUT_HOOK_ON);
