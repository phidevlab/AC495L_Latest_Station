...
/*
 *	This function behaves exactly as function RvSdpCodecFmtpParamGetByName2, the only 
 *  difference that multiple separator values can be given in zero-terminated string 
 *  pointed by 'separator'. RvSdpCodecFmtpParamGetByName2 will try each separator
 *  from string pointed by 'separator' in a sequential manner until one of them succeeds.
 */
RVSDPCOREAPI RvSdpStatus RvSdpCodecFmtpParamGetByName2(
    RvSdpMediaDescr* descr, 
    const RvChar* name,
    RvUint32 payload,
    RvChar* separator,
