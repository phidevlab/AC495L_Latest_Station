...
#define rvDeclareList(T) \
	...
	RVAPI RvList(T) *_RvList##T##Construct(RvList(T) *l, RvAlloc *a); \
	...
	RVAPI void _RvList##T##PopFront(RvList(T) *l); \
	...
	void _RvList##T##Splice(RvList(T) *d, RvListIter(T) it, RvList(T) *s, \
	  RvListIter(T) f, RvListIter(T) l); \
	RVAPI RvListIter(T) _RvList##T##Erase(RvList(T) *l, RvListIter(T) n); \
	RVAPI void _RvList##T##Clear(RvList(T) *l); \
...
#define rvDefineList(T) \
	...
	RVAPI RvListIter(T)  _RvList##T##Erase(RvList(T) *l, RvListIter(T) it) { \
		if (it != &l->end) { \
