...
**
** $Id: Si321x_Intf.c,v 1.43 2007/02/26 16:46:04 lajordan Exp $
**
** Proslic.c
**
** Author(s): 
...
/* NIRP 070606 instead of manually calibrating every init - use preconfigured parameters */
//#define ACL_USE_FACTORY_CALIBRATION_PARAMETERS 
...
static int powerUp(proslicChanType_ptr *pProslic,int size){ 
	...
	vBatTarget = ((uInt16)vBatProg*15)/10;  //target in volts
				...
				if (data < vBatTarget){
					...
					if (i > 200){
						if (pProslic[j]->debugMode)
							printk("Timed out during POWER UP %d %d\n", vBat, vBatTarget);
		...
		for (j=0;j<size;j++){
			if (pProslic[j]->channelEnable){
				...
				while (pRamTable[i].address != 0xffff){
                              	pProslic[j]->WriteRAMX(pProslic[j]->pProHWX,pProslic[j]->channel,pRamTable[i].address,pRamTable[i].initValue);
					i++;
				}
			}
		}
	...
	if (pRegTable != 0){
		...
		do {
				...
				while (pRegTable[i].address != 0xff){
					pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel,pRegTable[i].address,pRegTable[i].initValue);//broadcast
					i++;
				}
		} while (++j < size);
	}
...
int Si321x_Init (proslicChanType_ptr *pProslic, int size){
	...
	*/
	...
	for (j=0;j<size;j++){
		...
		if (data !=2) {
			if (pProslic[j]->debugMode)
				LOGPRINT("ProSLIC not communicating a) - channel =%d i- data = 0x%x.\n", pProslic[j]->channel, data);
	...
	for (j=0;j<size;j++){
		...
		if (data !=0x33) {
			if (pProslic[j]->debugMode)
				LOGPRINT("ProSLIC not communicating b) - channel =%d - data = 0x%x.\n", pProslic[j]->channel, data);
			pProslic[j]->error = SPIFAIL;
			pProslic[j]->channelEnable = 0;     
                     return (-1);
		}
              else
                    	pProslic[j]->channelEnable = 1;  
	}
	...
	printk("finsih reading type and revision, #channel = %i, chipRev = %i, chipType = %i\n", pProslic[0]->channel, pProslic[0]->deviceId->chipRev , pProslic[0]->deviceId->chipType);	

	/*STEP 9 AN35*/
   
	Si321x_LoadRegTables(pProslic,(ProslicRAMInit *)ramInitTable, NULL, size);
...
#ifndef ACL_USE_FACTORY_CALIBRATION_PARAMETERS 
    Si321x_Cal(pProslic,size); //STEPS 14-20
#else
    Si321x_Cal_ACL(pProslic,size); //NIRP 070606 use preconfigured calibration parameters
#endif
    //----------------------------
    ...
    Si321x_LoadRegTables(pProslic,(ProslicRAMInit *)ramInitTable2, (ProslicRegInit *)regInitTable,size);
...
int Si321x_Cal(proslicChanType_ptr *pProslic, int size){
	//uInt8 x,y,progress=0; // progress contains individual bits for the Tip and Ring Calibrations
	int i=0;
	uInt8   orData;
	int32 timeOut,nCalComplete;
	uInt32 manCalStat=0;
	int j;
	int ch = pProslic[0]->channel;
       //int errorCode=0;
	//-----------------------------
	...
	for (j=0;j<size;j++){
		if (pProslic[j]->channelEnable){
			if (pProslic[j]->ReadRegX(pProslic[j]->pProHWX,pProslic[j]->channel,LOOP_STAT) & 1){ //offhook
				if (pProslic[j]->debugMode)
					LOGPRINT("Calibration failure due to off-hook phone\n");
...
/* NIRP 070606 instead of manually calibrating every init - use preconfigured parameters */
int Si321x_Cal_ACL(proslicChanType_ptr *pProslic, int size){
	int j;
	int ch = pProslic[0]->channel;
	printk("%s channel %d\n", __FUNCTION__, ch);
	j=0;
	do {
		switch (ch){
			...
			pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 98, 0x17);
                	pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 99, 0x16);
               		pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 100, 0x12);
	                pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 101, 0x12);
        	        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 102, 0x1);
        	        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 103, 0x42);
			pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 105, 0x2B);
			pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 107, 0x9);
#elif defined(CONFIG_TULIP2_40)
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 98, 0x1C);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 99, 0xD);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 100, 0x14);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 101, 0x14);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 102, 0x6);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 103, 0x44);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 105, 0xD7);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 107, 0x8);
#endif
			break;
			case 1:
#if defined(CONFIG_TULIP2_21)
			pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 98, 0x18);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 99, 0xD);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 100, 0x11);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 101, 0x17);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 102, 0x0);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 103, 0x22);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 105, 0x27);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 107, 0x9);
#elif defined(CONFIG_TULIP2_40)
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 98, 0x19);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 99, 0x18);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 100, 0xF);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 101, 0x13);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 102, 0x4);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 103, 0x63);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 105, 0xD6);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 107, 0x9);
#endif
			break;
			case 2:
			pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 98, 0x16);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 99, 0x15);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 100, 0x14);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 101, 0x13);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 102, 0x8);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 103, 0x63);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 105, 0xD8);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 107, 0x9);
			break;
			case 3:
			pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 98, 0x17);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 99, 0x17);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 100, 0x14);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 101, 0x19);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 102, 0x5);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 103, 0x40);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 105, 0x2A);
                        pProslic[j]->WriteRegX(pProslic[j]->pProHWX,pProslic[j]->channel, 107, 0x8);
			break;
			default:
				break;
...
int Si321x_ToneGenSetup (proslicChanType *pProslic, int preset){
	Si321x_Tone_Cfg *pTone = Si321x_Tone_Presets;
	if (pProslic->deviceId->chipType == SI3215 || pProslic->deviceId->chipType == SI3215M)
...
int Si321x_ZsynthSetup (proslicChanType *pProslic, int preset){
    ...
    WriteReg(pProHW, pProslic->channel, LINE_IMPEDANCE, Si321x_Zsynth_Presets[preset].clc | Si321x_Zsynth_Presets[preset].tiss);
	//NIRP 070627 according to Silabs there is a bug in this code, for the time being - commented out
    //See Email from Richard in this matter
    //WriteReg(pProHW, pProslic->channel, 11, Si321x_Zsynth_Presets[preset].clc);
    return 0;
}
//NIRP 070628 support RGS
int ACL_Si321x_GetCountryIndex(char* CountryName)
{
    int i;
    
    //Go over the impedance structure and look for the country
    for (i=0; Si321x_Zsynth_Presets[i].name!= NULL; i++)
    {
        if (!strcmp(Si321x_Zsynth_Presets[i].name, CountryName))
        {
    	    //LOGPRINT("ACL_Si321x_GetCountryIndex: Found a match (country=%s, index=%d) .\n", CountryName, i);
            return i;
        }

    }
    //No match
//    LOGPRINT("ACL_Si321x_GetCountryIndex: No match, return default value .\n");
    return 0;
}
...
int Si321x_PulseMeterSetup (proslicChanType *pProslic, int preset){
	...
	WriteReg(pProHW, pProslic->channel, PULSE_ON__LO, (Si321x_PulseMeter_Presets [preset].pat));//pulse metering active timer
	WriteReg(pProHW, pProslic->channel, PULSE_ON__LO+1, (Si321x_PulseMeter_Presets [preset].pat)>>8);//pulse metering active timer
	WriteReg(pProHW, pProslic->channel, PULSE_OFF_LO, (Si321x_PulseMeter_Presets [preset].pit));//pulse metering inactive timer
	WriteReg(pProHW, pProslic->channel, PULSE_OFF_LO+1, (Si321x_PulseMeter_Presets [preset].pit)>>8);//pulse metering inactive timer
	return 0;
}
