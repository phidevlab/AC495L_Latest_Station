                                                 ...
                                                 PAL_VLYNQ_ENDIAN_ENUM_T peer_endianness)
{
    ...
    if((p_reg->revision >= 0x00010200) && local_endianness)
    {
    ...
    if((p_reg->revision >= 0x00010200) && peer_endianness)
				            ...
				            Uint8                      peer_clock_div)
{
    ...
    p_vlynq_reg->control &= ~(0x1 << 15);
    p_vlynq_reg->control &= ~(0x7 << 16);
    p_vlynq_reg->control |= (local_clock_dir << 15) | ((local_clock_div - 1) << 16);
    ...
    p_vlynq_reg->control &= ~(0x1 << 15);
    p_vlynq_reg->control &= ~(0x7 << 16);
    p_vlynq_reg->control |= (peer_clock_dir << 15) | ((peer_clock_div - 1) << 16);
...
{
    PAL_VLYNQ_ISR_INFO_T *p_isr_info;
    VLYNQ_REG_T *p_reg = (VLYNQ_REG_T*)(p_vlynq->base + 0x80);
...
#ifdef VAG115_PEER_LE_TO_BE_SWAP /* SSI May not be required. */
{
    Uint32 swapped_val;
    swapped_val     = vlynq_endian_swap(p_reg->control);
    swapped_val    &= ~(0x7f80);
    p_reg->control  = vlynq_endian_swap(swapped_val);
}
#else
    p_reg->control  &= ~(0x7f80);
#endif
...
PAL_VLYNQ_HND *PAL_vlynqInit(PAL_VLYNQ_CONFIG_T *pal_vlynq_config)
{
    ...
    /* Do we have the link on the VLYNQ bridge on reset when we are seeking
     * to sink in clock. */
...
char* vlynq_get_popular_name(Uint32 id, char *soc_name)
{
    ...
    switch(id)
    {
        ...
        case 0x0006:
            name = "vdsp";
            break;
#if defined(CONFIG_MIPS_TITAN) || defined(TNETV1050SDB)
        case 0x0007: 
            name = "titan";
            break;
#else
...
#ifdef VAG115_PEER_LE_TO_BE_SWAP
                                  limit - len, vlynq_endian_swap(p_reg->revision));
#else
                                  limit - len, p_reg->revision);
#endif
     *eof = 1;
