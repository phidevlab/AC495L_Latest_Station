...
/*
 * RvSipCSeqHeaderFieldName
 * ----------------------------
 * This enum defines all the CSeq header fields.
 * It is used for getting and setting via RvSipHeader interface.
 */
typedef enum
{
	RVSIP_CSEQ_FIELD_STEP   = 0,
    RVSIP_CSEQ_FIELD_METHOD = 1,
	RVSIP_CSEQ_FIELD_BAD_SYNTAX = 2
}RvSipCSeqHeaderFieldName;
 ...
 * Arguments:
 *    hHeader - Handle to the CSeq header object.
 *    pStep   - Pointer to an unsigned variable, to be filled with the 
 *				CSeq step value, only if it's already initialized.
 ***************************************************************************/
