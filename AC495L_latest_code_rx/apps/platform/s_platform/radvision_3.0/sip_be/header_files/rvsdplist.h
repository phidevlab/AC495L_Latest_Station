...
/* this function is called for each LListNode when LList instance is compared
   to other LList instance*/
typedef void* (*rvSdpListCompareFunc)(void* dest, void* src);
...
/* to compare two list instances.
 * returns RV_TRUE if they are equal */
RvBool rvSdpListCompare(
        const RvSdpList* l1,
        const RvSdpList* l2,
        rvSdpListCompareFunc cf); /* this function will be called
                                     for each compared list node */
