 ...
 * RVSIP_SUBS_EVENT_PACKAGE_TYPE_PRESENCE       - The event package is presence package.
 * RVSIP_SUBS_EVENT_PACKAGE_TYPE_PRESENCE_WINFO - The event package is presence.winfo package. Note
 *                                                that the Event header that gives rise to this package
 *                                                contains an event package presence and an event template
 *                                                winfo.
...
typedef enum
{
    ...
    RVSIP_SUBS_EVENT_PACKAGE_TYPE_PRESENCE,
    RVSIP_SUBS_EVENT_PACKAGE_TYPE_PRESENCE_WINFO,
...
/* RVSIP_SUBS_EXTRA_PROTECTION_EXPIRES_VAL
 * -----------------------
 * Defines a default expiration value to be used by the subscription for extra protection. 
 * The extra protection expires value is used in the following cases:
 * 1. The polling-resource case: RFC 3265 defines that polling-resource request (initial 
 *    SUBSCRIBE with expires 0) should be responded with 200 and a NOTIFY request, with 
 *    "Subscription-State:terminated;reason=timeout". If this NOTIFY is arrived, the 
 *    subscription will be terminated as it should. If this NOTIFY is not arrived, the 
 *    no-notify timer will terminate the subscription. The extra protection is for the 
 *    case that a NOTIFY with "Subscription-State:active" is arrived. In this case the 
 *    alert timer expiration will cause the subscription termination.
 * 2. In case that there is no expiration value defined for a subscription, i.e. there
 *    was no expiration value in the SUBSCRIBE, 200 and NOTIFY, the subscription will use
 *    the extra protection expires value as its expiration value.
 * The extra protection expires value is given in seconds
 */
#define RVSIP_SUBS_EXTRA_PROTECTION_EXPIRES_VAL 60
 ...
 *          Application should use this callback to send a refresh when the 
 ...
 *           At this stage the application should use the RvSipSubsAuthProceedExt()
 *           function.
 ...
 * Return Value: RV_OK 
 *                 Otherwise - will cease the handling of the received message
 * ------------------------------------------------------------------------
 * Arguments:
 * Input:     hSubs -      The sip stack subscription handle
 *            hAppSubs -   The application handle for this subscription.
