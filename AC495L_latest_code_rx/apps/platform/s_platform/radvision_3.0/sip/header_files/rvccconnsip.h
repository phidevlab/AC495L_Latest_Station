...
typedef struct
{
	...
	RvUint16                callAuthRetriesCount;   /* how many times we have responded to consequent 401/407 msgs */
	RvMtfOfferAnswerState   offerAnswerState;
	RvMtfUpdateState        updateState;
	RvBool                  updateAllowedByRemoteParty;
	IppTimer                updateResendTimer;    /* resend UPDATE when this timer expires */
	RvSdpMsg*               sdpOfLastSentUpdate;  /* store the last sent UPDATE SDP in case we need to resend the message */
} RvCCConnSip;
...
void rvCCConnSipRingback(RvCCConnection* x, RvCCEventCause reason);
...
void rvCCConnSipSetUpdateState(IN RvCCConnection* x,IN RvMtfUpdateState updateState);
RvMtfUpdateState rvCCConnSipGetUpdateState(IN RvCCConnection* x);
