...
typedef struct {
    ...
    uint16 packet;      /**< Packet events mask */
...
typedef enum {
    VP_DEV_EVID_BOOT_CMP        = 0x0001, /**< DVP/VPP Startup sequence has
...
typedef enum {
    ...
    VP_STREAM_2 = 0x0002,
    VP_STREAM_3 = 0x0003,
    VP_STREAM_ENUM_SIZE = FORCE_STANDARD_C_ENUM_SIZE /* Portability Req. */
} VpPktMailboxEventDataType;

typedef enum {
    VP_RING_CAD_BREAK,          /* Start of OFF period of the ringing cadence */
    VP_RING_CAD_MAKE,           /* Start of ON period of the ringing cadence */
    VP_RING_CAD_DONE,           /* End of specified ringing cadence */
    VP_RINC_CAD_ENUM_SIZE = FORCE_STANDARD_C_ENUM_SIZE /* Portability Req. */
...
typedef enum {
    VP_DIG_SENSE_BREAK  = 0x00,
    VP_DIG_SENSE_MAKE   = 0x10,
    VP_DIG_SENSE_ENUM_SIZE = FORCE_STANDARD_C_ENUM_SIZE /* Portability Req. */
} VpDigitSenseType;

typedef enum {
    VP_POLREV_NORMAL,   /* Line detected Reverse-to-Normal Transition */
    VP_POLREV_REVERSE,  /* Line detected Normal-to-Reverse Transition */
 ...
 *                In case of DVP, VPP class of devices, response mailbox is 
