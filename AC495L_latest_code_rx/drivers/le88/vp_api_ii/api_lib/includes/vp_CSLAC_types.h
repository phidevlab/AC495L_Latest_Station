...
#define VP_CSLAC_MAX_PROF_TABLE_SIZE            11
...
typedef struct {
    ...
    uint8 acProfEntry;
    uint8 dcProfEntry;
    uint8 ringingProfEntry;
    uint16 toneCadProfEntry;
    uint16 toneProfEntry;
    uint8 ringCadProfEntry;
    uint8 meterProfEntry;
    uint8 cidCadProfEntry;
    uint8 fxoConfigProfEntry;
    uint8 customTermProfEntry;    
...
typedef enum {
    VP_DEV_INIT_IN_PROGRESS = 0x0001,   /**< Set if device init in progress */
...
typedef struct {
    uint8 timing;       /**< Number of currently active timers */
...
typedef enum {
    ...
    VP_CSLAC_RINGING = 0x0004,    
    VP_CSLAC_DISC = 0x0008,
...
typedef struct {
    VpLineStateType currentType;    /**< Current state of the line */
