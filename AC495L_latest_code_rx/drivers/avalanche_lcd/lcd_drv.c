...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
    struct class *tilcd_class; /* For registering with sysfs fs */
#else
...
static int tilcd_probe_dev(struct platform_device *dev)
{
    return 0;    
}

static struct platform_driver tilcd_driver = {
...
static long tilcd_ioctl( struct file * file,
...
static int __init tilcd_init(void)
{
  ...
  if(PAL_osMemAlloc(0, sizeof(LCD_DEV), 0, ( void *)&lcd_dev) != PAL_SOK) 
  {
    ...
    tilcd_driver.driver.bus = platform_bus_type_ptr;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
    lcd_dev->tilcd_class = class_create (THIS_MODULE, lcd_dev->devname);
    ...
    if (device_create (lcd_dev->tilcd_class, NULL, lcd_dev->lcd_device_number,
	...
	proc_create("avalanche/lcd_ver", S_IRUGO | S_IWUSR, NULL, &lcd_proc_fops);
    return 0;
exit_init:
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
    if (lcd_dev->lcd_device_number > 0)
        device_destroy(lcd_dev->tilcd_class, lcd_dev->lcd_device_number);

    if (lcd_dev->tilcd_class)
        class_destroy(lcd_dev->tilcd_class);
#endif

    if (lcd_dev->lcd_device_number > 0)
        unregister_chrdev (MAJOR(lcd_dev->lcd_device_number), lcd_dev->devname);
...
MODULE_PARM_DESC(rows,"lcd rows");
module_param(columns, int, 0);
MODULE_PARM_DESC(columns,"lcd columns");
