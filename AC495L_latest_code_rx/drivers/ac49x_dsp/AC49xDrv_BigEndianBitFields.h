...
typedef volatile struct
    ...
    U16 RoundTrip_MswMsb				   : 8;  /*!< Always 0 \96 isn\92t calculated in RTCP transmit.*/
    U16 RoundTrip_MswLsb				   : 8;
    U16 RoundTrip_LswMsb				   : 8;
    U16 RoundTrip_LswLsb				   : 8;
...
typedef volatile struct
     ...
     * function ::Ac49xRfc3267AmrInitializationConfiguration. The last policy\92s fields (::Tac49xRfc3267CodecModeAttr) 
     * define this event\92s threshold.
     */
...
typedef volatile struct
	...
	U16											: 8;
	U16 VoiceCoderOfReceivedPackets				: 8;  /*!< The voice decoder that decodes packets coming from the network. The decoder is used to calculate voice quality. Refer to ::Tac49xCoder. */

	U16 IntervalMosLq							: 8;  /*!<MOS-LQ (Mean Opinion Score - Listening Quality) calculated during the last interval (which ended when the last event was received). The MOS value is not influenced by other time intervals.  */
	U16 IntervalMosCq							: 8;  /*!<MOS-CQ (Mean Opinion Score - Conversational Quality) calculated during the last interval (which ended when the last event was received). The MOS value is not influenced by other time intervals. */
...
typedef volatile struct
     ...
     * @n Refer to <A HREF="Packetizer Error \96 Parameters.html">Packetizer Error</A> table.
     */
...
typedef volatile struct
    ...
    U16 TimeStamp_LswLsb                : 8;  /*!< 32-bit time stamp*/
		/* 5 */
	U16	UsbReceiveSamplesOverrun_Msb	: 8;	
	U16	UsbReceiveSamplesOverrun_Lsb	: 8;	
		/* 6 */
	U16	UsbReceiveSamplesUnderrun_Msb	: 8;	
	U16	UsbReceiveSamplesUnderrun_Lsb	: 8;	
		/* 7 */
	U16	UsbTransmitSamplesOverrun_Msb	: 8;	
	U16	UsbTransmitSamplesOverrun_Lsb	: 8;	
		/* 8 */
	U16	UsbTransmitSamplesUnderrun_Msb	: 8;	
	U16	UsbTransmitSamplesUnderrun_Lsb	: 8;	
		/* 9 */
	U16	UsbReceiveCorrectionInVoice_Msb	: 8;	
	U16	UsbReceiveCorrectionInVoice_Lsb	: 8;	
		/* 10 */
	U16	UsbTransmitCorrectionInVoice_Msb: 8;	
	U16	UsbTransmitCorrectionInVoice_Lsb: 8;	
		/* 11 */
	U16	UsbFrameLossCount_Msb			: 8;	
	U16	UsbFrameLossCount_Lsb			: 8;	
